class Web::FairParticipantsController < Web::ApplicationController
  def index
    ...
    @users = UserDecorator.decorate_collection User.contest_year(2016).where("state = 'waiting_confirmation' OR state = 'fair_participant_approved'").where.not(role: :admin)
